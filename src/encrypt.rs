...
impl std::convert::From<&PrivateKey> for PublicKey {
    fn from(pk: &PrivateKey) -> Self {
        let private_key = agreement::EphemeralPrivateKey::generate(
            &agreement::X25519,
            &ring::test::rand::FixedSliceRandom { bytes: &pk.0 },
        )
        ...
        PublicKey(public_key.as_ref().to_owned())
    }
}
...
pub fn decrypt_share(share: &[u8], key: &PrivateKey) -> Result<Vec<u8>, EncryptError> {
    ...
    let ephemeral_pub = agreement::UnparsedPublicKey::new(&agreement::X25519, empheral_pub_bytes);
...
mod tests {
    ...
    fn test_encrypt_decrypt() -> Result<(), EncryptError> {
        let pub_key = PublicKey::from_base64("O0i1yoSjsdGC3Bb2yct/5hPkN6nzl1VE5560DGP7MVw=")?;
        let priv_key = PrivateKey::from_base64("5uUNlJLVcGaX7NBO24MdbA/b9TKy3Z3ILGq4ktWSYKE=")?;
